...
Config.setup = function(dir, widget, configExtension, callback)  
  ...
  return {
    ...
    save = function(data)
      local sel = context.storage._configs[dir] and context.storage._configs[dir].selected
        if not sel or tostring(sel) == "" then
          sel = "default"
          if context.storage._configs[dir] then context.storage._configs[dir].selected = sel end
        end
        Config.save(dir, sel, data, configExtension)
    end,
    refresh = refresh,
    reload = refresh,
