...
function bindWalkKey(key, dir)
    if not key or key:lower():find("num") then return end
...
function unbindWalkKey(key)
    local gameRootPanel = modules.game_interface.getRootPanel()
    if not gameRootPanel then return end
    pcall(g_keyboard.unbindKeyDown, key, gameRootPanel)
    pcall(g_keyboard.unbindKeyUp, key, gameRootPanel)
    pcall(g_keyboard.unbindKeyPress, key, gameRootPanel)
end
